...
def getMeshNumberFromFilename(numberOfMeshes, file):
    """Get mesh number string from file name
    
    Parameters
    ----------
    numberOfMeshes : int
        Integer containing the total number of meshes in the FDS simulation
    file : str
        String containing the filename of the boundary file generated by FDS
    
    Returns
    -------
    int
        Integer containing the mesh number from the filename
    """
    if numberOfMeshes == 1:
        meshNumber = 0
    else:
...
def queryBndf(resultDir, chid, fdsFilePath, fdsQuantities,
              axis, value, decimals=4, limits=None, debug=False):
    ...
    debug: bool
        Flag which enables printing of options per file for debugging
    ...
    if all_quantities_found is False:
        print("Not all quantities found. Available quantites are:")
        for q in quantities:
            print("\t%s"%(q))
        return datas, None
    
    # Find data at queried axis and value combination
    ...
    if all_locations_found is False:
        all_values = []
        ...
        for file in bndfFiles:
            ...
            if debug: print("Options for IOR %0.0f in %s"%(axis, file))
            if abs(axis) == 1:
                options = [opt[0][0] for opt in xoptions if opt[1] == axis]
                prefix = '\tx=\t'
            elif abs(axis) == 2:
                options = [opt[0][2] for opt in yoptions if opt[1] == axis]
                prefix = '\ty=\t'
            elif abs(axis) == 3:
                options = [opt[0][4] for opt in zoptions if opt[1] == axis]
                prefix = '\tz=\t'
            values = np.round(options, decimals=decimals)
            if debug:
        ...
        all_values.sort()
        values_str = ','.join([str(x) for x in all_values])
        if debug is False:
            print("Options for IOR %0.0f:"%(axis))
            print(prefix + values_str)
        return datas, None
    else:
        tmax = len(ts_out)
