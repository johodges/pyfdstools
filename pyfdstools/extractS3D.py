...
def extractS3dValues(resultDir, chid):
...
def encodeS3dData(data, quantity, dx):
    ...
    if quantity == 'HRRPUV':
        MAX_SMV = 1200
        encoded_data = np.array((data/MAX_SMV)*254, dtype=np.uint8)
    if quantity == 'TEMPERATURE':
        MAX_SMV = 2000
        TMPA = 20 + 273.15
        TMPM = 273.15
        TMP_MIN = TMPA-TMPM
        encoded_data = np.array(254*(data - TMP_MIN)/(MAX_SMV-TMP_MIN), dtype=np.uint8)
    return encoded_data
